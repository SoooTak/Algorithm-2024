...
# 6. 특정 버스가 지나가는 경유 정류장 목록을 반환하는 함수
def print_via_stops(city_code, route_id):
    ...
    if response.status_code == 200:
        ...
        for item in root.iter('item'):
            nodenm_element = item.find('nodenm') # 정류소명
            updowncd_element = item.find('updowncd') # 상 하행 구분
...
# 7. xml 데이터를 넘겨받아 노선 정보를 출력하는 함수
